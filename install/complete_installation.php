...
################################################################################
	...
	if($passed_step == 5){
		...
		$folder_root     	 = isset($_SESSION['folder_root']) 		? stripslashes($_SESSION['folder_root']) : '';
		$site_url     		 = isset($_SESSION['site_url']) 		? stripslashes($_SESSION['site_url']) : '';
					...
					if(EI_CHECK_DB_MINIMUM_VERSION && (version_compare($db->GetVersion(), EI_DB_MINIMUM_VERSION, '<'))){
						...
						if($sql_dump != ''){
							if(false == ($db_error = apphp_db_install($sql_dump_file))){
								...
								$config_file = str_replace('<FOLDER_ROOT>', $folder_root, $config_file);
